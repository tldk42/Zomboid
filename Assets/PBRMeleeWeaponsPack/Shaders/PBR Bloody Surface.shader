...
Shader "PBR Bloody Surface"
{
    ...
    SubShader
    {
        Tags
        {
            "RenderType" = "Opaque" "Queue" = "Geometry+0" "RenderPipeline" = "UniversalPipeline"
        }
        Cull Back
        HLSLINCLUDE
        ...
        inline float4 TriplanarSamplingSF(sampler2D topTexMap, float3 worldPos, float3 worldNormal, float falloff,
                                          float tilling, float3 normalScale, float3 index)
        {
        ...
        void surf(Input i, inout SurfaceOutputStandard o)
        {
            ...
            float temp_output_107_0 = (exp((triplanar130.x * _BloodAmount)) * (saturate(ase_vertex3Pos.y) *
                _BloodAmount));
            float BloodMask74 = saturate(temp_output_107_0);
            float3 lerpResult73 = lerp(tex2DNode17,
                                       BlendNormals(UnpackNormal(tex2D(_BloodNormalMap, uv_BloodNormalMap)),
                                                    tex2DNode17), BloodMask74);
